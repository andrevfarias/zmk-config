 ...
 * layout-redesign). Serve de demonstração e de ponto de partida — espelha
 * config/corne.keymap e docs/LAYOUT.md.
 */
import type { Bundle, Combo, FunctionalDoc, KeyBinding, VisualDoc } from './types'
...
const HOLD: Record<number, string> = {
  12: '⇧', 13: '⌃', 14: '⇧', 15: '⌥', 16: '❖',
  19: '❖', 20: '⌥', 21: '⇧', 22: '⌃',
  36: 'tap=trava', 38: 'tap=trava', 39: 'tap=trava', 41: 'tap=trava',
}
const NUM = [
  '*', '/', '9', '8', '7', E, E, '7', '8', '9', '/', '*',
  '-', '.', '6', '5', '4', '=', '=', '4', '5', '6', '.', '-',
...
const FN = [
  ...
  '⏮', 'Mute', 'F6', 'F5', 'F4', 'F11', 'F11', 'F4', 'F5', 'F6', 'Mute', '⏮',
...
// ---- combos ------------------------------------------------------------------
let seq = 0
function par(name: string, action: string, group: string, l: number[], r: number[],
  description?: string): Combo[] {
  const mk = (keys: number[], lado: string): Combo =>
    ({ id: `c${seq}_${lado}`, name, action, group, keys, description })
  seq++
...
const COMBOS: Combo[] = [
  ...
  ...par('BASE', 'base_reset', 'layers', [14, 16], [19, 21],
    'desliga todas as funções; preserva QWERTY/COLEMAK'),
  ...
  { id: 'cfg', name: 'CONFIG ⇄', action: 'toggle CONFIG', group: 'layers', keys: [0, 11] },
...
export function defaultBundle(): Bundle {
  ...
  const put = (pos: number, layer: string, b: KeyBinding) => {
    const k = (f.keys[String(pos)] ??= {})
    k[layer] = b
  }
  for (let i = 0; i < 42; i++) {
    const base: KeyBinding = { tap: QWERTY[i] }
    if (SHIFT[i]) base.shift = SHIFT[i]
    if (HOLD[i]) base.hold = HOLD[i]
    put(i, 'qwerty', base)
    if (COLEMAK[i] !== QWERTY[i]) put(i, 'colemak', { tap: COLEMAK[i] })
    if (NUM[i]) put(i, 'num', { tap: NUM[i] })
    if (FN[i]) put(i, 'fn', { tap: FN[i] })
    if (NAV[i]) put(i, 'nav', { tap: NAV[i] })
    if (PROG[i]) put(i, 'prog', { tap: PROG[i] })
    if (NORM[i]) put(i, 'norm', { tap: NORM[i] })
    if (CONFIG[i]) put(i, 'config', { tap: CONFIG[i] })
  }
